...
for num in n_topics:
    ...
    nmf = NMF(n_components=num, random_state=rand).fit(td_matrix)
...
for num in n_topics:
    print("Performing LDA for %d topics..." % num)

    # Time LDA
    start_time_LDA = time.time()
    model = lda.LDA(n_topics=num, n_iter=200, random_state=rand)
